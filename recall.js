...
(function recallBootstrap(window) {
  ...
  // Local Requires
  if (fs.existsSync(path.join(lt.objs.plugins.user_plugins_dir, 'Ewen'))){
    var localRoot = path.join(lt.objs.plugins.user_plugins_dir, 'Ewen'); //When installed through the plugin manager, the Recall folder is created with an upper case
  }
  else
  {
    var localRoot = path.join(lt.objs.plugins.user_plugins_dir, 'ewen'); //When recalled is manually cloned from the repository, the recall folder is created with a lower case
  }
